...
class _PageListReportsState extends State<PageListReports> {
  ...
  Widget _buildListUserBody() {
    return Container(
      child: FutureBuilder(
        future: ApiDataSourceReports.instance.loadReports(),
        builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot){
          ...
          if(snapshot.hasData){
            ...
            return _buildSuccesSection(dataReports);
          }
  ...
  Widget _buildItemUsers(Results dataReports){
    return InkWell(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => DetailReports(reportsData: dataReports,),
        ),
      ),
      child: Padding(
            ...
            child: Row(
              ...
              children: [
                Container(
                  width: 100,
                  height: 100,
                  child: Image.network(dataReports.imageUrl!),
                ),
                SizedBox(width: 20,),
                SizedBox(
                    width: 250,
                    height: 40,
                    child: Text(dataReports.title!)
                )
              ],
            ),
          ),
